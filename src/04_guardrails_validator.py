...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        """
        Tìm PII trong value; nếu phát hiện, redact và trả về FailResult(fix_value=redacted_text).
        """
        redacted_text = value
        found_pii = []
...
class JSONFormatter(Validator):
    ...
    def _repair(text: str) -> str:
        ...
        # 1. Gỡ markdown code fences
        text = re.sub(r"```(?:json)?\s*", "", text)
        text = re.sub(r"```\s*$", "", text, flags=re.MULTILINE)
        text = text.strip()

        # 2. Sửa single quotes thành double quotes
    ...
    def validate(self, value: str, metadata: dict):
        ...
        Nếu vẫn không sửa được, trả về fallback error JSON.
        """
        ...
        # Không thể sửa — trả về JSON lỗi chuẩn (fallback)
        fallback = json.dumps({
...
def demo_json_guard():
    ...
    for label, text in test_cases:
        result = guard.validate(text)
        status = "✅ Pass" if result.validation_passed else "🔧 Fixed via Fallback"
        print(f"\n[{label}] {status}")
